...
import random
import pandas as pd
# from HNdataPatch_cliparaVec_multilabel import ReadCTData,GetClinicalVec,GetDataDirs
...
def main():
    ...
    file = open(opt.result_path + '/model_G.txt', 'a+') # generator
    file.write(str(txt_G))
    file.close()
    
    file = open(opt.result_path + '/model_D.txt', 'a+')  # discriminator
    ...
    latent_feature_test_csv.to_csv(opt.result_path+'/latent_feature_test.csv') # extracted latent features for test set.
    ...
    latent_feature_csv.to_csv(opt.result_path+'/latent_feature.csv') # extracted latent features for all patients (training and test sets)
