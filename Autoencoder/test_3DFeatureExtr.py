...
def test(epoch, data_loader, model_G,model_D, criterion_G,criterion_D, opt,
                epoch_logger,writer):
    ...
    with torch.no_grad():
        for i, inputs in enumerate(data_loader):
            ...
            if (opt.model == 'resnet') or (opt.model == 'resnet_fm') or (opt.model == 'resnet_fm_pyramid_sum_Jiapan') or (opt.model == 'resnet_fm_pyramid_sum_CIT') or (opt.model == 'resnet_fm_pyramid_concat_Jiapan') or (opt.model == 'resnet_fm_pyramid_concat_CIT'):
                 #recons, outputs_latent , feature_maps = model_G(inputs)
                 ...
                 d_loss  = criterion_D(logits_fake.detach(),label_0 ) + criterion_D(logits_real,label_1 )
            ...
            writer.add_scalar('ADV/val', g_loss_dict['ADV'].item(), n_iter)
            writer.add_scalar('DICE/val', g_loss_dict['DICE'].item(), n_iter)
            writer.add_scalar('D_Loss/val', d_loss.item(), n_iter)
