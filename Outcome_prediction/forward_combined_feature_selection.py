...
def main():
    ...
    for event_cols,survival_cols in zip([event_columns_code[opt.outcome]],[survival_columns[opt.outcome]]):
        ...
        savename = './forward_selection/'+str(opt.sub)+str(event_cols)+'_combine.csv'
    ...
    csv = pd.read_csv('./forward_selection/'+str(opt.sub)+
                      event_columns_code[opt.outcome]+'_combine.csv',header=0)
    for sub in range(2,11): 
        savename = opt.result_path_ct+'/forward_selection/'+str(sub)+event_columns_code[opt.outcome]+'_combine_specific.csv'
    ...
    csv.to_csv('./forward_selection/total_'+event_columns_code[opt.outcome]+'_combine.csv', index = True, header=True)
    '''
